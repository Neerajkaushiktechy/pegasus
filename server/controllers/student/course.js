...
exports.getCourse = async (req, res) => {
  try {
    let data;
    if (req.params.id) {
      data = await StudentCourse.findById({ _id: req.params.id }).collation({ locale: "en" }).sort({ coursename: 1 });
    } else {
      if (req.roleId === 1) {
        data = await StudentCourse.find({}).collation({ locale: "en" }).sort({ coursename: 1 });
      }
      else {
        ...
        }).collation({ locale: "en" }).sort({ coursename: 1 });
      }
    }
    if (!data) {
      return res
