...
import { postLaboratoryRequestAction, updateLaboratoryRequest } from "../../../../../../redux/modules/patients/laboratory/action";
...
export default function Laboratory({ editData, patientId, assessmentId, submittedTime, assignmentId, checkAssignment = false }: props) {
   ...
   const handleSubmit = (e: any) => {
      ...
      if (!formData.onlyView) {
         if (editData?._id) {
            dispatch(updateLaboratoryRequest(formData));
         } else {
            dispatch(postLaboratoryRequestAction(formData));
         }
      }
   };
   ...
   let { getLaboratory, postLaboratory, updateLaboratory } = useSelector((state: any) => {
      let { getLaboratory, postLaboratory, updateLaboratory } = state;
      return { getLaboratory, postLaboratory, updateLaboratory };
   });
   ...
   useEffect(() => {
      if (postLaboratory.data?.success) {
         setShowAlert({ show: true, title: postLaboratory.data?.message })
         postLaboratory.data = postLaboratory.initialState.data;
         if (assessmentId) {
            let status = 2
      ...
      if (updateLaboratory.data?.success) {
         setShowAlert({ show: true, title: updateLaboratory.data?.message })
         updateLaboratory.data = updateLaboratory.initialState.data;
         dispatch(fetchPatientInformationRequest(DemographicRes?.pId));
      }

   }, [dispatch, postLaboratory.data?.success, updateLaboratory.data?.success, showListData, setShowListData, DemographicRes]);
         ...
         <Paper>
            {!checkAssignment &&
               <Stack p={2} direction="row" justifyContent="space-between" alignItems="center" spacing={2} bgcolor="primary.light" sx={{ padding: "6px 20px" }}>
                  <Typography>Add Laboratory</Typography>
                  {patientId ? (
                     ""
                  ) :
                     <Button
                  ...
                  <Grid container spacing={2}>
                     <Grid item xs={12}>
                        <FormControl variant="standard" fullWidth>
                           <InputLabel shrink htmlFor="laboratory">
                              Laboratory
                           </InputLabel>
                           <Input
                              sx={{ padding: "14px" }}
